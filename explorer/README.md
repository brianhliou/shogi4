...
A local, browser-based board for **Shogi4** — play through games move by move on a
board backed by the project's validated rules engine. Friendly-jumps, far-rank
promotion, drops, and king-capture wins are all generated by the engine, not the
page.
...
computed, `/api` gains each move's result + distance-to-mate and the status bar
becomes a verdict bar — the rest is unchanged.
...
No dependencies (Python stdlib only). The server imports the validated oracle
`engine/shogi4.py` for move generation — the viewer writes no second engine.
...
- **Recorded lines:** the dropdown loads the rule demos and a sample game from
  `clips.json`. Deep-link one with `?game=<id>` (e.g. `?game=friendly-jump`).
...
`/api?pos=<enc>` returns the position's legal moves. The encoding matches
...
Each move carries its child position (for navigation), `from`/`to` squares,
readable `notation`, and `capture` / `promo` / `jump` / `terminal` flags.

## Pieces of this directory

| file | what |
|---|---|
| `index.html` | the single-file SVG viewer (vanilla JS) |
| `serve.py` | stdlib HTTP server: `/`, `/clips.json`, `/pieces/*.png`, `/api?pos=` over the oracle |
| `clips.json` | oracle-generated demo + sample lines (regenerate via `tools/gen_clips.py`) |
...
The viewer's correctness rests on the oracle (move generation) plus a thin codec.
`python3 explorer/test_codec.py` re-derives the legal moves for all 4,000 golden
positions through the viewer's codec and asserts they match the oracle's recorded
sets (and that positions round-trip), so no divergent move logic can creep in.
